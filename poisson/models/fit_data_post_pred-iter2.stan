...
transformed parameters {
    ...
    for(i in 1:N){
        ...
        lambda[i] = early[i] * failure_form(phi, age[i]) + wear[i] * failure_form(rho, -age[i] + 1) + eta[ship_number[i]];
...
generated quantities{
    ...
    for(i in 1:N_pred){
        ...
        real lambda_pred = early_pred * failure_form(phi, age_pred[i]) + wear_pred * failure_form(rho, -age_pred[i] + 1) + eta[ship_number_pred[i]];
