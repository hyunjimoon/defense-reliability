...
library(rstan)
library(ggplot2)
model_DMsep <- stan_model(file.path(getwd(), "optimize/step1/DM_5param.stan"), verbose = FALSE) #approx_deterioration_matrix
...
opt_data <- list(N= length(imputed_data$y_data), n_state=n_state, state_obs=onehot_array, time_obs=imputed_data$age_ind, repair_state=repair_state, initial_state=initial_state)
