...
generated quantities{
  ...
  vector<lower=1, upper = max_interval>[4] i; // interval
  real cost[max_interval,max_interval,max_interval,max_interval]; #check init to 0 : rep_matrix(0, max_interval, max_interval,max_interval,max_interval);
  ...
  //   }
  // }
  for(i[1] in 1:max_interval){ // interval 1
    for(i[2] in 1:max_interval){// interval 2
      for(i[3] in 1:max_interval){ // interval 1
        for(i[4] in 1:max_interval){// interval 2
          if (sum(interval) > 20) { #TODO could be improved
            cost[i[1],i[2],i[3],i[4]] = positive_infinity();
            continue;
          }
          for (t in 1:max(time_obs)){
            if (t == 1){
              ...
              if (t - 9 * int(t/9) % intveral == 0){
                  state_t[t] = M * state_t[t];
                  state_t_NM = t_p[t] * state_t[t-1];
                  cost[i1, i2] = cost[i1, i2] + inspection_cost 
                                              + pm_cost * (state_t[t] - state_t_NM)[pm_init:(cm_init - 1] + cm_cost * (state_t[t] - state_t_NM)[cm_init:];
                }
            }
          }
        }
