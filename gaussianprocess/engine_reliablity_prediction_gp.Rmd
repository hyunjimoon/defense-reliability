...
1) Increase `sigma_error_ship` 
@is decreasing `sigma_error_ship` a better approach when underfitting, i.e. smaller variance is observed compared to the real data?
```{stan}
...
MSE for this model is `r mse_res_underfit_1` where original(`sigma_error_ship ~ normal(0.5,1)`) was `r mse_res_reparam`. MSE didn't decrease by increasing `sigma_error ship`.


2) Give weaker `sigma_error_ship` prior 

```{stan}
sigma_error_ship ~ normal(0,1) ->  sigma_error_ship ~ normal(0,0.5)
```
