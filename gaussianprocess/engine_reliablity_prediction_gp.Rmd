...
mse_res_reparam <- mseNplot(sf_reparam_sm,y)
mse_res_reparam
```
...
mse_res_reparam_5var <- mseNplot(sf_reparam_5var_sm,y)
mse_res_reparam_5var 
```
...
We approached the underestimated $\tilde{y}$ variance problem in two different ways.

1) Larger value of `sigma_error_ship`

The variance of  $\tilde{y}$  looks quite larger than fitted values(i.e. underfitting). So we simply gave larger value of `sigma_error_ship` prior@.

```{stan, eval=F, output.var="md"}
sigma_error_ship ~ normal(0,1) -> sigma_error_ship ~ normal(1.5,1)
```
...
mse_res_underfit_1 <- mseNplot(sf_underfit_1_sm,y)
mse_res_underfit_1
```

MSE for this model is `r mse_res_underfit_1` where original(`sigma_error_ship ~ normal(0,1)`) was `r mse_res_reparam`. MSE didn't decrease by increasing `sigma_error ship`.


2) Smaller value of `sigma_error_ship` 

Since prior of `sigma_error_ship` is half normal, giving larger variance is equivalent to smaller expected value of half normal prior@. 

```{stan, eval=F, output.var="md"}
sigma_error_ship ~ normal(0,1) ->  sigma_error_ship ~ normal(0,3)
```
...
mse_res_underfit_2 <- mseNplot(sf_underfit_2_sm,y)
mse_res_underfit_2
```
