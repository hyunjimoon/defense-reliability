...
mseNplot <- function(x, y){
  ...
  par(mfrow=c(1,1))
  plot(1, type="n",xlim=c(0,31),ylim=c(-3,4),xlab="age",ylab="scaled failure time")
  for (n in 1:653){
...
- remove prop prior
- unify trend from long and short trend
- parameterize the `length_engine_ship` prior parameters
...
# 2. prior pushforward check

## 2.1. divergence

> length_GP_engine ~ weibull(30,6); // value?
  length_GP_ship ~ weibull(30,6);
  
> length_GP_engine ~ weibull(length_engine_shape,length_engine_scale); // effect last for 8year -> 6 year 
  length_GP_ship ~ weibull(length_ship_shape,length_ship_scale);
  
Prametrizing prevented the need for our domain knowledge but instead, we had to set reasonable prior for `length_GP_engine` and `length_GP_ship`. First attemps were just to experiment with the given parmameters; which ended up with divergences.
> length_engine_shape ~  normal(30, 1);
  length_engine_scale ~ normal(8, 1);
  length_ship_shape ~  normal(30, 1);
  length_ship_scale ~ normal(3, 1);
...
div_detect(rsf_hp)
```
Two modification: give smaller prior to `length_GP_engine` and reparameterize.

```{r}
emp_le_shape <-sf_hp$summary("length_engine_shape")%>% pull(mean)
emp_le_scale <-sf_hp$summary("length_engine_scale")%>% pull(mean)
emp_ls_shape <-sf_hp$summary("length_ship_shape")%>% pull(mean)
emp_ls_scale <-sf_hp$summary("length_ship_scale")%>% pull(mean)
print(c(emp_le_shape, emp_le_scale, emp_ls_shape,emp_ls_scale))
```
As can be seen from the result, the value does not change much. This might becase, data, due to security issues, are scaled and therfore have rather weak impact on the prior update. Therefore, to investigate the parameters' true range, weak priors are given. We chose $lognormal(0,1)$.

> length_engine_scale ~ lognormal(0, 1);
  length_ship_scale ~ lognormal(0, 1);
  length_engine_shape ~  lognormal(0, 1);
  length_ship_shape ~  lognormal(0, 1);
...
we decided to restrict the range of `length_engine_scale` based on the empirical values.
...
```
No divergence. We set this value as our prior parameters.
...
## 2.2. correlated samples and uneven density

When plotted, `length_GP_engine` and `length_engine_scale` showed ununiform samples which can be problematic, so we decided to reparameterize `length_GP_engine`.

```{r}
partition <- util$partition_div(rsf_hp_n)
div_samples <- partition[[1]]
nondiv_samples <- partition[[2]]
...
```{r}
writeLines(readLines('models/gp_reparam/gp_reparam.stan'))
```
...
It shows uniform sample distribution and there's no linear relationship between two parameter. 

# 3.prior predictive check

## 3.1 Different $sigma$ for each engine

We plotted predicted values of $y$ using above reparametrized model, but data seems different to the predicted values.

```{r}
sf_reparam_sm <- sf_reparam$summary()

mseNplot(sf_reparam_sm,y)
```

For each lifetime, standard deviation for each engine looks quite different. For example, engine 4 and 5 shows large variance while engine 3 shows small variance of given data.
...
```{r}
writeLines(readLines('models/gp_reparam_5var/gp_reparam_5var.stan'))
```

```{r}
modelName <- "gp_reparam_5var"

sf_reparam_5var <- gp_fit(modelName, data)
rsf_reparam_5var <- read_stan_csv(sf_reparam_5var$output_files())
util$check_all_diagnostics(rsf_reparam_5var)
div_detect(rsf_reparam_5var)

sf_reparam_5var_sm<-sf_reparam_5var$summary()

mseNplot(sf_reparam_5var_sm,y)
```

But mse value didn't actually decrease, so we decided to use same variance for each engine. 

## 3.2 Underfitting

Even we gave different $sigma$ for each engine, the variance looks quite larger than fitted values. (i.e. underfitting) So we tried to fix this problem with two different approach.

1) Simply increase sigma_error_ship (simple comparison between fitted vs observed)

> sigma_error_ship ~ normal(0,1) 
...
```{r}
modelName <- "gp_underfit_1"

sf_underfit_1 <- gp_fit(modelName, data)
rsf_underfit_1 <- read_stan_csv(sf_underfit_1$output_files())
util$check_all_diagnostics(rsf_underfit_1)
div_detect(rsf_underfit_1)

sf_underfit_1_sm<-sf_underfit_1$summary()

mseNplot(sf_underfit_1_sm,y)
```

MSE for this model is 0.2559 where original(`sigma_error_ship ~ normal(0.5,1)`) was 0.2456. MSE didn't decrease by increasing `sigma_error ship`.


2) Give small `sigma_error_ship` prior to prevent underfitting 
...
```{r}
modelName <- "gp_underfit_2"

sf_underfit_2 <- gp_fit(modelName, data)
rsf_underfit_2 <- read_stan_csv(sf_underfit_2$output_files())
util$check_all_diagnostics(rsf_underfit_2)
div_detect(rsf_underfit_2)

sf_underfit_2_sm<-sf_underfit_2$summary()

mseNplot(sf_underfit_2_sm,y)
```

This gives smaller MSE so we choose this model.


# Conclusion

Finally, we gave *normal* prior to `length_engine_scale`,`length_ship_scale`, `length_engine_shape`, `length_ship_shape` with mean gained from *lognormal* prior.
