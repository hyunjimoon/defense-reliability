...
parameters {
    real<lower=1> phi[ship_number_max];
    real<lower=1> rho[ship_number_max];
...
transformed parameters {
    ...
    for(i in 1:N){
        ...
        lambda[i] = early[i] * failure_form(phi[ship_number[i]], -age[i]+1) + wear[i] * failure_form(rho[ship_number[i]], age[i]) + eta[ship_number[i]];
