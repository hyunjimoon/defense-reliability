functions{
    real failure_form(real shape, real scale, real age){
        if(shape == 0 || scale == 0){
            //print("failure_form ", shape, " ", scale, " ", age);
            return 0;
        }
        return fmax(fmin((shape/scale) * pow(age/scale, shape-1) * exp(-pow(age/scale, shape)), 2), 0.01);
...
transformed parameters {
    ...
    for(i in 1:N){
        ...
        lambda[i] = /*failure_form(alpha[ship_number[i]], beta[ship_number[i]], age[i]) + */failure_form(gamma[engine_type[i]], delta[engine_type[i]], age[i]) * wear[i];

        lambda[i] = exp(lambda[i]);
        /*if(lambda[i] <= 0 || is_nan(lambda[i])){
            print("ERROR ERROR", lambda[i], " ", wear[i], " ", etta[engine_type[i]], " ", theta[ship_number[i]]);
        }*/
    }
}
