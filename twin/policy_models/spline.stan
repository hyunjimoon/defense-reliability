...
transformed parameters { 
    ...
    if(is_nan(s_a) || is_inf(s_a)){
        print("s_a is nan or inf");
    }
  
    for (n in 1: N){
        for(k in 1:K){
            if(is_nan(w[ship[n]][k]) || is_inf(w[ship[n]][k])){
                print("w is nan or inf");
            }
        }
        if(is_nan(a[ship[n]]) || is_inf(a[ship[n]])){
